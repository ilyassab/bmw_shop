...
const Desktop: FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await useData(client, 'ACCESSORIES')
  ...
  useEffect(() => {
    const fetchData = async () => {
      const data = await useCategoryData(client, 'ACCESSORIES', category)
      setCategories(data)
    }
    fetchData()
  }, [])
  ...
  return (
    <Column flexGrow={1}>
      ...
      >
        <Breadcrumbs
          secondText={messages.accessories}
          secondHref='/catalog/accessories'
          thirdText={title}
          onThirdClick={() => {
      ...
      <Layout justifyContent='center' backgroundColor='white'>
        <Column width='283px'>
          <FiltersCategory block='accessories' />
          <ResetFilters />
          <Filters block='accessories' />
        </Column>
        <Layout flexBasis='24px' />
        <Column maxWidth={['90%', '90%', '900px']} width='100%'>
          <CategoryBmwBlock />
          <Layout flexBasis='24px' />
          <CatalogGrid block='accessories' />
        </Column>
      </Layout>
