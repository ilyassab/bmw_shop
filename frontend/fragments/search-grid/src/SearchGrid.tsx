...
/** @ts-ignore */
const SearchGrid: FC<WrappedComponentProps & Props> = ({ intl, search }) => {
  ...
  const fetchQuery = async () => {
    try {
      setLoading(true)
      const data = await useData(client, search, 8, currentPage, decodeURI(query.sort || '') || '')
      if (!state.fetchMore) {
        setState({
          ...state,
          items: data.items,
...
export default injectIntl(SearchGrid)
