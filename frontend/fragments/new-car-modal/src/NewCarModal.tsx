...
const NewCarModal: FC<Props> = ({ onClose, opened, setCars }) => {
  const intl = useIntl()
  ...
  const onClick = async () => {
    ...
    const data = await useDataPostCar(
      client,
      {
      ...
      >
        ...
        <Column maxWidth={['90%', '90%', '672px']} width='100%'>
          ...
          <Layout maxWidth='90%'>
            ...
            >
              {intl.formatMessage(messages.newCar)}.
            </Text>
          </Layout>
          ...
          <Box width='100%' flexDirection={['column', 'row', 'row']}>
            ...
            <BMWBlockSelect
              ...
              value={currentDealer.name || intl.formatMessage(messages.dealers)}
              onClick={item => {
