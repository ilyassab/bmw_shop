...
const FavoriteGrid: FC<WrappedComponentProps & Props> = ({ intl }) => {
  ...
  const fetchQuery = async () => {
    ...
    try {
      setLoading(true)
      const data = await useData(client, 8, currentPage, decodeURI(query.sort || '') || '', token)
      if (!state.fetchMore) {
        setState({
          ...state,
          items: data.items,
...
export default injectIntl(FavoriteGrid)
